...
This server configuration provides the ability to download media using
BitTorrent (via with VPN support) and/or Usenet. All media is automatically
organized and served on the network by Plex Media Server.
...
Optionally, you can install `avahi` to enable connections using the server's
hostname:
...
recommended), remove the `network_mode` line from the `qbittorrent` service in
`docker-compose.yml`. Configuration updates will need to be made to Prowlarr,
Radarr, and Sonarr download client settings, by pointing the server to
`qbittorrent`, rather than `wireguard`.
...
Update the `[Interface]` section of your config file to allow incoming traffic
from the host (update the `HOMENET` IP as needed for your network)
([Source](https://www.linuxserver.io/blog/routing-docker-host-and-container-traffic-through-wireguard#routing-docker-container-traffic-through-wireguard)):
...
instructions in [this repository](https://github.com/hsand/pia-wg#linux-debianubuntu)
...
<details>
<summary>qBittorrent settings</summary>
...
<details>
  <summary>SABnzbd settings</summary>
...
<details>
<summary>Prowlarr settings</summary>
...
<details>
  <summary>Bazarr settings</summary>
...
<details>
  <summary>Overseerr settings</summary>
...
<details>
  <summary>Tautulli settings</summary>
...
the `monitoring/docker-compose.yml` from the main `docker-compose.yml` file.

| Service | Description |
| ------- | ----------- |
| <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/3/38/Prometheus_software_logo.svg/2066px-Prometheus_software_logo.svg.png" alt="prometheus" width="12"/>  Prometheus | Monitoring system and time series database |
...
Remote access to the server from the internet can be configured using dynamic
DNS and a reverse proxy.
...
forward ports 80 (HTTP) and 443 (HTTPS) to the server/ports where Nginx Proxy
Manager service is listening. See the [Nginx Proxy Manager
documentation](https://nginxproxymanager.com/guide/#hosting-your-home-network)
for more details.

![port-forward](https://i.imgur.com/7Lzy6Yn.png)

You can now test if external access is working by visiting one of your
subdomains from outside the local network.
